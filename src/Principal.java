...
public class Principal {
	...
	private static void solucionarPuntoG(List<Punto> listaPunto) {
		...
		List<Punto> listaG = listaPunto.stream().filter(p -> Math.abs(p.getY() - Fx.fx(p.getX())) < 3)
	...
	private static void solucionarPuntoH(List<Punto> listaPunto) {
		...
		List<Punto> listaH = listaPunto.stream().filter(p -> p.getX() >= 0)
				.filter(p -> p.getY() >= 0 && p.getY() <= Fx.fx(p.getX())).collect(Collectors.toList());
		System.out.println(graficarLista(listaH));
		System.out.println("--- < -------------- > ---\n");
	}

	private static void solucionarPuntoJ(List<Punto> listaPunto) {
		System.out.println("--- Lista ejercicio J ---\n");

		List<Punto> listaJ = listaPunto.stream().filter(punto -> punto.getX() >= -4 && punto.getX() <= 4)
				.filter(punto -> punto.getY() > Fx.fx(punto.getX())).collect(Collectors.toList());
